...
export default function ProfEditor({ prof, onSave, onCancel }: ProfEditorProps) {
  ...
  return (
    <motion.div
      ...
      exit={{ opacity: 0, y: -20 }}
      className="max-w-2xl mx-auto bg-cream rounded-2xl shadow-xl border border-warm overflow-hidden"
    >
      {/* Header */}
      <div
      ...
      <div className="p-6 space-y-8">
        ...
        <div className="space-y-2">
          ...
          <textarea
            ...
            rows={4}
            className="w-full bg-white/60 border border-warm rounded-xl p-4 text-sm text-ink leading-relaxed placeholder:text-muted/40 focus:border-accent focus:ring-1 focus:ring-accent/30 outline-none resize-none transition-colors"
          />
        </div>
        ...
        <div className="space-y-3">
          ...
          <div className="relative">
            <input
              ...
              className="w-full bg-white/60 border border-warm rounded-lg px-4 py-2.5 text-sm text-ink placeholder:text-muted/40 focus:border-accent outline-none transition-colors"
            />
            ...
            <AnimatePresence>
              {data.attributes.map((attr) => (
                <motion.div
                  ...
                  className="flex items-center gap-3 bg-white/60 rounded-xl px-4 py-3 border border-warm/60"
                >
                  <input
                    type="text"
                    value={attr.label}
            ...
            <div className="flex flex-wrap gap-2">
              ...
              {unusedAttributeSuggestions.map((label) => (
                <button
                  ...
                  className="flex items-center gap-1 px-2.5 py-1 rounded-full text-[0.65rem] font-mono bg-warm/50 text-ink hover:bg-warm transition-colors cursor-pointer border border-warm"
                >
        ...
        <div className="flex justify-end gap-3 pt-4 border-t border-warm">
          <button
            type="button"
            onClick={onCancel}
            className="flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-medium text-muted bg-warm/50 hover:bg-warm transition-colors cursor-pointer"
          >
